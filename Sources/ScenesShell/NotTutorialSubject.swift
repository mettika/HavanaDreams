...
class noTutorialSubject : RenderableEntity, EntityMouseClickHandler {
    var Width: Int
    var Height: Int
    ...
    func DrawTutorialBox(canvasSize: Size, canvas: Canvas) {
        var BoxRectangle = DrawRectangle(x: 3 * canvasSize.center.x / 2, y: 7 * canvasSize.center.y / 4, Width:175, Height:50)
        var fillStyle = FillStyle(color:Color(.black))
        var strokeStyle = StrokeStyle(color:Color(.black))
        let  text = Text(location:Point(x: (canvasSize.center.x / 2) + 5, y: (7 * canvasSize.center.y / 4) + 30), text: "No Tutorial")
        ...
        if !containment.intersection([.containedFully]).isEmpty {
            print("\(globalLocation.x) Degrees North, \(globalLocation.y) Degrees East. READY TO FIRE.")
