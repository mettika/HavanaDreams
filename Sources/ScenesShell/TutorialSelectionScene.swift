 ...
 */
class TutorialSelectionScene : Scene {
     ...
     */
    let backgroundLayer = TutorialBackgroundLayer()
    let interactionLayer = TutorialInteractionLayer()
    let foregroundLayer = TutorialForegroundLayer()
    let subjectLayer = TutorialSubjectLayer()
    ...
    init() {
        ...
        insert(layer:foregroundLayer, at:.inFrontOf(object:interactionLayer))
        insert(layer:subjectLayer, at:.front)
    }
}
